...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "first-jbpm-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name first-jbpm-service apps/first-jbpm-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app first-jbpm-service:1.0-SNAPSHOT
    oc expose svc/first-jbpm-service
else

	echo "Launching the application locally..."
	pattern="first-jbpm-service"
	files=( $pattern )
	cd ${files[0]}
