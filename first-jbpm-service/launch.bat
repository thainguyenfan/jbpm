...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "first-jbpm-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name first-jbpm-service apps/first-jbpm-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app first-jbpm-service:1.0-SNAPSHOT
		call oc expose svc/first-jbpm-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd first-jbpm-service
		cd target
